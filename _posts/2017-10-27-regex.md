  ...
  * `(?=(?:[^"]|'[^']*'|"[^"]*")*$)`, this could be further split into:
    ...
    * `(?:[^"]|'[^']*'|"[^"]*")` which could further split into:
      ...
      * `|` or conditions listed below
        1. `[^"]` complement of set `["]`
	2. `'[^']*'` pattern with one single quote plus 0 or more non-single quote with a ending single quote, or in simpler English, a pair of single quote with some or zero ordinary character in between, e.g. 'single'.
	3. `"[^"]*"` pattern with one double quote plus 0 or more non-double quote with a ending double quote, or in simpler English, a pair of double quote with some or zero ordinary character in between, e.g. "couple".
